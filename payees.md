...
수취인(Payee)은 [Programmable Wallet](user-wallet.md)에서 결제를 받을 수 있도록 미리 승인된 주소입니다. 단, 그 범위는 사용자가 정한 정확한 한도 안으로 제한됩니다. 시장 데이터 피드, 모델 제공자, 컴퓨트 벤더, 외주 인력, 급여, 구독 서비스처럼 반복적으로 결제해야 하는 수취인을 미리 승인해 두세요. 결제당 한도, 기간 한도, 허용 자산, 쿨다운을 설정할 수 있습니다. 자금은 실제로 필요한 마지막 순간까지 DeFi에서 계속 수익을 냅니다.
...
Underscore: 벤더를 Payee로 추가 → 월 $5,000 한도 → USDC만 → 분기마다 자동 만료
```
...
**소유자 직접 통제**: [소유자 지갑](user-wallet.md)인 사용자만 Payee를 직접 추가, 확정, 확장할 수 있습니다. 매니저는 새 Payee를 제안할 수는 있지만, 그 제안은 설정된 지연이 지난 뒤 소유자가 승인하기 전까지 보류 상태로 남습니다. Payee는 스스로를 제거할 수는 있지만, 소유자 외에는 누구도 활성화하거나 한도를 넓힐 수 없습니다. 이는 정책이 아니라 스마트 컨트랙트로 강제됩니다.

### 변경에 강한 수취인 기록

**온체인 수취인 기록**: Payee가 설정되면 수취인 주소와 한도는 온체인에 저장됩니다. 매니저가 몰래 "GPU Provider"를 다른 주소로 바꾸거나 소유자 승인 없이 한도를 넓히는 일은 불가능합니다.
...
각 Payee마다 포괄적인 제어를 설정해, 결제가 의도한 방식대로만 일어나게 만들 수 있습니다. 이런 제어는 과지출, 타이밍 오류, 무단 변경을 막아 줍니다.

### 재무 한도

#### 이중 보호 시스템

모든 Payee는 토큰 수량 기준과 USD 가치 기준 한도를 모두 지원합니다.
...
- **만료일**: 정해진 시점이 지나면 Payee 자동 비활성화 (예: 외주 계약 종료일)

#### 자산 제한

각 Payee가 어떤 토큰을 받을 수 있는지 정확히 제어할 수 있습니다.
...
- **Self-Payment 토글**: 자기 자신에게의 결제 허용/차단
- **기본 한도**: 새 Payee가 상속받는 기본 설정

#### 개별 오버라이드

- **사용자 정의 한도**: 특정 관계에 대해 글로벌 설정을 덮어쓰기
- **풀 결제 권한**: 글로벌에서 꺼져 있어도 Payee별로 허용 가능
- **자산 화이트리스트**: 글로벌 설정보다 더 좁은 토큰 제한

### 시스템 한도

- **최대 Payee 40개**: 월렛당 수용량
- **최대 자산 40개**: Payee별 제한 목록
...
**소유자**: 어떤 Payee든 추가, 확정, 수정, 제거할 수 있는 완전한 권한  
**매니저**: 새 Payee 제안 가능 (지연 후 소유자 승인 필요)  
**기타 주체**: Payee나 그 한도를 변경할 권한 없음

### 누가 결제를 실행할 수 있나요

**소유자**: 한도 안에서 어떤 활성 Payee에게든 결제 가능  
**매니저**: 매니저 한도와 수취인 한도를 모두 만족하는 경우에만 활성 수취인에게 결제 가능  
**수취인**: 명시적으로 허용된 경우 풀 결제 가능

### 누가 제거할 수 있나요

**소유자**: 어떤 Payee든 즉시 제거 가능  
**수취인**: 스스로를 제거 가능 (정리된 종료 방식)  
**매니저**: Payee를 직접 제거할 수 없음
...
- 한도 안에서 활성 Payee에게 결제 가능
- 새 Payee를 제안할 수 있음 (사용자 승인 필요)
...
> 매니저가 새 Payee를 제안하면 보류 중 요청이 생성됩니다. 사용자가 설정한 보안 지연이 지난 뒤, 사용자가 직접 확정해야 Payee가 활성화됩니다. 이 2단계 프로세스는 승인되지 않은 수취인 추가를 막아 줍니다.

## 풀 결제(Pull Payment): 수취인이 시작하는 선택형 인출

Payee가 사용자의 월렛에서 직접 자금을 인출하도록 허용할 수 있습니다. 단, 언제나 사용자가 미리 정한 한도 안에서만 가능합니다.

### 작동 방식

1. Payee가 사용자 월렛에서 인출을 시작합니다
...
**매니저**([매니저](managers.md))는 자신의 권한과 예산 안에서 승인된 결제 흐름을 실행합니다.
