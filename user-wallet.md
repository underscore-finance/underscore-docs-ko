---
description: 프로그래머블 결제와 DeFi 접근을 지원하는 에이전트형 월렛 — 수탁 없이 실행 권한만 부여
---
...
Underscore의 Programmable Wallet은 AI 에이전트와 운영자가 온체인 규칙 안에서 실행할 수 있도록 설계된 스마트 컨트랙트 월렛입니다. [매니저](managers.md)는 권한을 정의하고, [수취인](payees.md)은 반복 지출을 처리하며, [디지털 수표](cheques.md)는 선택적 검토 지연이 있는 일회성 결제를 처리합니다. 모든 것은 오픈소스 스마트 컨트랙트에 의해 온체인에서 강제됩니다.
...
Underscore는 "레고"라고 부르는 표준화된 어댑터를 통해 DeFi 프로토콜에 연결됩니다. 각 레고는 프로토콜별 작업을 일관된 인터페이스로 감싸며, 최적화된 가스 사용량으로 원자적 멀티프로토콜 트랜잭션을 가능하게 합니다.

**이 월렛은 미래에도 그대로 유효합니다**. 새로운 DeFi 프로토콜이 출시되거나 기존 프로토콜에 기능이 추가되면, 새 레고 통합을 통해 자동으로 월렛에 반영됩니다. 자금을 옮길 필요도, 새 월렛을 배포할 필요도, 무언가를 업데이트할 필요도 없습니다. 새 레고가 배포되는 순간 기존 Programmable Wallet도 바로 새 프로토콜에 접근할 수 있게 됩니다.
...
Ripe Protocol은 Underscore의 대출 파트너로, 자산을 팔지 않고도 유동성을 확보할 수 있게 해줍니다. 핵심 장점은 **Underscore Earn Vaults가 담보로 인정된다는 점**입니다. 즉, 자산이 AI 최적화 수익을 내는 동안 그 자산을 담보로 대출도 받을 수 있습니다. 수익과 유동성 중 하나를 고르는 것이 아니라 둘 다 가져갈 수 있습니다.

**작동 방식**:

1. **담보 예치**: 자산을 Ripe Protocol에 잠급니다. Underscore Earn Vault 토큰도 포함됩니다
2. **대출 실행**: GREEN(스테이블코인) 또는 수익형 sGREEN으로 대출받습니다
3. **수익 유지**: Vault 토큰을 담보로 쓰는 경우, 기초 자산은 계속 최적화 수익을 창출합니다
...
- Earn Vault에 예치해 AI가 수익을 최적화하게 합니다
- 그 볼트 지분 토큰을 Ripe에서 담보로 사용합니다
...
- 지원되는 어떤 자산이든 담보로 예치
- Underscore Earn Vault 토큰을 생산적인 담보로 활용
- GREEN(스테이블코인) 또는 sGREEN(수익형 스테이블코인) 대출
- 허용된 어떤 토큰으로든 부채 상환
- 부채 포지션에서 RIPE 보상 획득 및 청구

**매니저 권한**:
- **Manage Debt**: 담보 추가/제거, 대출, 상환
...
매니저는 **Manage Liquidity** 권한으로 유동성 작업을 처리할 수 있습니다.
