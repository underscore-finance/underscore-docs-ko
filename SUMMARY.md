# 목차

## 시작하기

- [Underscore Protocol](README.md) — 당신의 돈은 잠들지 않습니다
- [Underscore를 사용하는 곳](who-uses-underscore.md) — 에이전트, 앱, 프로토콜을 위한 인프라
- [DeFi 통합](defi-integrations.md) — 지원 프로토콜

## Earn Vaults

- [Earn Vaults](vaults.md) — AI가 관리하는 ERC-4626 수익 전략

## Programmable Wallets

- [Programmable Wallets](user-wallet.md) — 프로그래머블 결제와 DeFi 접근을 지원하는 에이전트형 월렛
- [Managers](managers.md) — 에이전트에게 실행 권한을 부여하되, 수탁 권한은 주지 않습니다
- [Payees](payees.md) — 에이전트가 필요한 서비스 비용을 결제하도록 합니다
- [디지털 수표](cheques.md) — 무조건적인 신뢰 없이 처리하는 일회성 지출
- [화이트리스트](whitelist.md) — 신뢰하는 주소로 즉시 무제한 전송

## 이코노믹스

- [보상](rewards.md) — 수익 보너스와 예치를 통해 RIPE 토큰 획득
- [프로토콜 이코노믹스](protocol-economics.md) — 수수료가 어떻게 작동하고 어디로 가는지

## 개발자

- [기술 문서](https://underscore-1.gitbook.io/developers) — 스마트 컨트랙트 가이드와 통합 문서
- [배포된 컨트랙트](https://underscore-1.gitbook.io/developers/home/deployments) — Base 상의 컨트랙트 주소
- [GitHub](https://github.com/underscore-finance/underscore-protocol) — 오픈소스 저장소
