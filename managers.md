...
# Managers: 에이전트에게 실행 권한을 부여하되, 수탁 권한은 주지 않습니다

대부분의 에이전트는 추천만 할 수 있습니다. Underscore는 프라이빗 키를 절대 넘기지 않고도 실제 자금으로 실행하게 만듭니다.

매니저는 [Programmable Wallet](user-wallet.md)을 대신해 전략을 실행하고 결제를 처리하는 승인된 운영자입니다. 하지만 완전한 통제권은 절대 갖지 않습니다. 예를 들어 "USDC는 최대 $50k까지 거래하되 ETH는 건드리지 마라", "Aave, Morpho, Euler, Compound 사이에서만 리밸런싱하라", "$10k는 일일 한도, $100k는 월간 한도"처럼 정확한 권한을 설정할 수 있습니다. 자동 만료되는 시간 기반 제어를 넣을 수도 있고, 원하면 한 번의 클릭으로 즉시 권한을 철회할 수도 있습니다.
...
[Programmable Wallet](user-wallet.md)을 혼자 운영하면 피할 수 없는 트레이드오프가 생깁니다.
...
- **시간 지연**: 민감한 작업에는 선택적 쿨다운을 설정할 수 있습니다

### 2단계 보안 검사

모든 Manager 작업은 자동 검증을 거칩니다.
...
| 권한 | 기능 | 예시 사용 사례 |
| ---- | ---- | -------------- |
| **General Transfers** | [Payees](payees.md), [화이트리스트](whitelist.md), 또는 [디지털 수표](cheques.md)를 통한 자산 전송 | 월간 지출 결제 |
| **Create [Cheques](cheques.md)** | 일회성 결제 예약 | 지연이 있는 벤더 결제 |
| **Propose [Payees](payees.md)** | 반복 결제 수취인 추가 제안 | 새 외주 인력 온보딩 |

### DeFi 작업

| 권한 | 기능 | 예시 사용 사례 |
| ---- | ---- | -------------- |
| **Buy & Sell** | 토큰 스왑, 포트폴리오 리밸런싱 | ETH/USDC 60/40 비중 유지 |
| **Manage Yield** | 수익 프로토콜에 예치/인출 | Morpho/Aave 포지션 리밸런싱 |
| **Manage Debt** | 대출과 담보 관리 | 담보비율 150% 유지 |
| **Manage Liquidity** | DEX 유동성 공급/회수 | Uniswap V3 범위 최적화 |
| **Claim Rewards** | 프로토콜 인센티브 수확 | [파밍 보상](rewards.md) 청구 및 재투자 |

### 관리 작업

| 권한 | 기능 | 예시 사용 사례 |
| ---- | ---- | -------------- |
| **Whitelist Management** | 승인된 주소 추가/제거 | 벤더 목록 관리 |
| **Claim Protocol Rewards** | Underscore 인센티브 청구 | [플랫폼 보상](rewards.md) 자동 청구 |
| **Claim Loot** | 수익 공유 회수 | [프로토콜 수익](rewards.md) 극대화 |
...
권한이 매니저가 무엇을 할 수 있는지 정의한다면, 제어 장치는 그들이 어떤 범위 안에서만 움직일 수 있는지 결정합니다. 이 한도 덕분에 신뢰하는 Manager라 해도 사용자의 리스크 허용 범위를 넘거나 실수 혹은 악의적인 행동으로 월렛을 비우는 일은 불가능해집니다. 모든 Manager 작업은 이런 보안 체크포인트를 통과해야 합니다.
...
- 기간 길이는 글로벌 설정의 `managerPeriod`로 지정합니다 (예: 1일 = 43,200 blocks)
...
- 블록 수로 측정됩니다 (예: Base에서 1시간 = 1,800 blocks)
...
- 미리 승인된 주소로의 전송만 허용합니다
- 알려진 벤더만 상대하는 비즈니스 운영에 적합합니다
- 매니저당 승인된 [Payee](payees.md) 최대 40개
...
시스템은 글로벌 설정과 Manager 개별 설정 중 더 엄격한(더 낮은) 한도를 적용합니다.
...
Manager A: 거래 가능 = 예, 트랜잭션당 최대 = $50k → 실제 한도는 $50k
Manager B: 거래 가능 = 아니오, 트랜잭션당 최대 = $200k → 거래 자체 불가
```
...
- General Transfers: 승인된 [수취인](payees.md)으로만 전송
- Create [Cheques](cheques.md): 일회성 데이터 구매용
- 자산: USDC만
...
**설정**: 30명 이상의 외주 인력과 벤더를 둔 글로벌 크립토 기업, CFO를 Manager로 지정
...
- Manage Debt: 대출 상환, 담보 추가
- Manage Yield: 상환용 자금을 위해 Aave/Morpho에서 인출
...
- Buy & Sell: 주요 토큰 사이 스왑 허용
- Manage Yield: Aave/Morpho/Euler만 예치 가능
...
**Manager가 다른 Manager를 추가할 수 있나요?**  
아니요. 새 Manager를 추가할 수 있는 것은 월렛 소유자뿐입니다. 이를 통해 권한 상승 공격을 막습니다.

**Manager가 만료되면 어떻게 되나요?**  
모든 권한을 자동으로 잃습니다. 사용자가 별도로 할 일은 없습니다.

**Manager를 제거하지 않고도 한도를 바꿀 수 있나요?**  
